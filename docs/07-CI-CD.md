# PokerNotes — CI/CD Pipeline

Complete GitHub Actions pipeline for PokerNotes: quality gates, testing, preview deployments, and production deploys to Cloudflare Pages.
...
|---|---|---|
| `main` | Production-ready code | `pokernotes.app` (production) |
| `dev` | Integration branch | `dev.pokernotes.app` (staging) |
...
jobs:
  ...
  build:
    ...
    steps:
      ...
      - run: pnpm build
        env:
          NEXT_PUBLIC_APP_URL: https://pokernotes.app
    ...
    steps:
      ...
      - name: Deploy to Cloudflare Pages (preview)
        ...
        with:
          ...
          command: pages deploy .vercel/output/static --project-name=pokernotes --branch=${{ github.head_ref }}
    ...
    steps:
      ...
      - name: Apply D1 migrations (staging)
        ...
        with:
          apiToken: ${{ secrets.CLOUDFLARE_API_TOKEN }}
          accountId: ${{ secrets.CLOUDFLARE_ACCOUNT_ID }}
          command: d1 migrations apply pokernotes-db-staging --remote
      ...
      - name: Deploy to Cloudflare Pages (staging)
        ...
        with:
          apiToken: ${{ secrets.CLOUDFLARE_API_TOKEN }}
          accountId: ${{ secrets.CLOUDFLARE_ACCOUNT_ID }}
          command: pages deploy .vercel/output/static --project-name=pokernotes --branch=dev

      - name: E2E smoke tests
        run: pnpm test:e2e:smoke
        env:
          E2E_BASE_URL: https://dev.pokernotes.app
```
    ...
    steps:
      ...
      - name: Apply D1 migrations (production)
        ...
        with:
          apiToken: ${{ secrets.CLOUDFLARE_API_TOKEN }}
          accountId: ${{ secrets.CLOUDFLARE_ACCOUNT_ID }}
          command: d1 migrations apply pokernotes-db --remote
      ...
      - name: Deploy to Cloudflare Pages (production)
        ...
        with:
          apiToken: ${{ secrets.CLOUDFLARE_API_TOKEN }}
          accountId: ${{ secrets.CLOUDFLARE_ACCOUNT_ID }}
          command: pages deploy .vercel/output/static --project-name=pokernotes --branch=main

      - name: E2E smoke tests
        run: pnpm test:e2e:smoke
        env:
          E2E_BASE_URL: https://pokernotes.app
  ...
  lighthouse:
    ...
    steps:
      ...
      - uses: treosh/lighthouse-ci-action@v12
        with:
          urls: |
            https://dev.pokernotes.app/
            https://dev.pokernotes.app/tools/pot-odds
            https://dev.pokernotes.app/tools/icm
...
```json
{
  "scripts": {
    ...
    "db:migrate:local": "wrangler d1 migrations apply pokernotes-db --local",
    "db:migrate:staging": "wrangler d1 migrations apply pokernotes-db-staging --remote",
    "db:migrate:prod": "wrangler d1 migrations apply pokernotes-db --remote",
    "prepare": "husky"
  }
}
```
...
```markdown
![CI](https://github.com/USERNAME/pokernotes/actions/workflows/ci.yml/badge.svg)
![Deploy](https://github.com/USERNAME/pokernotes/actions/workflows/deploy-production.yml/badge.svg)
![E2E](https://github.com/USERNAME/pokernotes/actions/workflows/e2e-nightly.yml/badge.svg)
```
